 ...
 */
export function getDismissVelocity(
  event: PanGestureHandlerEventPayload,
  directions: PanningDirections[],
  options: TranslationOptions,
  ...
  const { velocityPassedThreshold, xPassedThreshold, yPassedThreshold } = checkThresholds(
    directions,
    velocity,
    _threshold,
    options,
  ...
  if (velocityPassedThreshold || xPassedThreshold || yPassedThreshold) {
    let _velocity: Partial<Frame> = {};
    if (velocityPassedThreshold) {
      _velocity = { x: event.velocityX, y: event.velocityY };
